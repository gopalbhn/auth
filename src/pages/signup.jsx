  ...
  return (
        ...
        <div className='bg-white p-10 rounded shadow-lg w-96'>
            <h2 className='text-2xl font-bold mb-5 text-center text-[#FF4000]'>Sign In</h2>
                ...
                <input type="email" placeholder='Full Name' className='w-full border border-gray-300 px-4 py-2 rounded focus:outline-none focus:ring-2 focus:ring-[#FF4000]' onChange={(e)=>setName(e.target.value)}/>
                ...
                <button type='submit' className='w-full bg-[#FF4000] hover:bg-white text-white  hover:text-[#FF4000] hover:border border-[#FF4000] px-4 py-2 rounded hover:cursor-pointer' onClick={handleClick}>Sign In </button>
                </div>
                <p>Already have an accout? <Link to="/signin" className='underline text-[#FF4000]'>Sign In</Link></p>
        </div>
    </div>
  )
}
